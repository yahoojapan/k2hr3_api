...
if [ ${IS_SCRIPT_MODE} -eq 1 ]; then
	...
	echo -n "            "
	${SRCTOP}/tests/auto_control_subprocess.sh -start -key server${KEYWORD} -int 3 chmpx -conf ${SRCTOP}/tests/auto_k2hdkc_server.ini -ctlport 18021 -d msg
	if [ $? -ne 0 ]; then
	...
	echo -n "            "
	${SRCTOP}/tests/auto_control_subprocess.sh -start -key server${KEYWORD} -int 3 k2hdkc -conf ${SRCTOP}/tests/auto_k2hdkc_server.ini -ctlport 18021 -d msg
	if [ $? -ne 0 ]; then
	...
	echo -n "            "
	${SRCTOP}/tests/auto_control_subprocess.sh -start -key slave${KEYWORD} -int 3 chmpx -conf ${SRCTOP}/tests/auto_k2hdkc_slave.ini -ctlport 18031 -d msg
	if [ $? -ne 0 ]; then
	...
	echo -n "            "
	${SRCTOP}/tests/k2hdkc_test_load.sh --for_auto_test
	if [ $? -ne 0 ]; then
	...
	echo -n "           "
	${SRCTOP}/tests/auto_control_subprocess.sh -stop -key slave${KEYWORD} -int 3 chmpx
	if [ $? -ne 0 ]; then
		echo "ERROR: chmpx slave node for k2hdkc"
		exit 1
	fi
	...
	echo -n "           "
	${SRCTOP}/tests/auto_control_subprocess.sh -stop -key server${KEYWORD} -int 3 k2hdkc
	if [ $? -ne 0 ]; then
		echo "ERROR: one k2hdkc process"
		exit 1
	fi
	...
	echo -n "           "
	${SRCTOP}/tests/auto_control_subprocess.sh -stop -key server${KEYWORD} -int 3 chmpx
	if [ $? -ne 0 ]; then
		echo "ERROR: could not run chmpx server node for k2hdkc"
		exit 1
	fi
