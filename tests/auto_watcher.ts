...
import	watchobj					from '../src/lib/ipwatch';			// Watcher process library
...
//
export function registerWatcherTests(): void {

	//--------------------------------------------------------------
	// Main describe section
	//--------------------------------------------------------------
	describe('PROCESS : WATHCER', function(){
		//
		// Common data
		//
		...
		//
		it('RUN WATCHER : run watcher process(lib) oneshot with result null', function(done){
			new Promise(function(resolve, reject)											// promise object is used
			{
				watchobj.watchAddressesAlive(function(result: boolean)
				{
		...
		//
		it('CHECK WATCHER RESULT : get auto_test_role role which has only two IP addresses with response 200', function(done){
			let	uri	= '/v1/role';
		...
		it('CHECK WATCHER RESULT : get k8s_test_role role which has only two IP addresses with response 200', function(done){
			let	uri	= '/v1/role';
