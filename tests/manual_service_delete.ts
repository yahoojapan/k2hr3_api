...
//
function rawDeleteV1Service(token: string, service: string, tenant: string | null): void
{
...
cliutil.getConsoleInput('Scoped user token for service owner tenant               : ', true, false, function(isbreak: boolean, token?: string | null): void
{
	...
	cliutil.getConsoleInput('Service name                                             : ', true, false, function(isbreak: boolean, service?: string | null): void
	{
		...
		cliutil.getConsoleInput('Remove tenant name(empty(null)->remove service)          : ', true, false, function(isbreak: boolean, tenant?: string | null): void
		{
			if(isbreak){
				process.exit(0);
			}
