...
//
export function start(parentobj: Mocha.Context): void {
	...
	//
	const orgTimeout = parentobj.timeout();
	parentobj.timeout(10000);

	//
	// Run chmpx for server node
	//
	let	result = execSync('tests/auto_control_subprocess.sh --start --key server --interval 3 -- chmpx -conf tests/auto_k2hdkc_server.ini -ctlport 18021 -d msg');
...
//
export function stop(parentobj: Mocha.Context): void {
	...
	//
	let	result = execSync('tests/auto_control_subprocess.sh --stop --key slave --interval 3 -- chmpx');
