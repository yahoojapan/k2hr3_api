...
//
function getV1UserData(registerpath: string, is_gzip: boolean): void
{
	...
	const	options = {
		'host':					hostname,
		'port':					hostport,
		'path': 				'/v1/userdata/' + encodeURI(registerpath),
		'method':				'GET',
		'headers':				headers,
...
cliutil.getConsoleInput('Response Type (gzip or text(default)) : ', true, false, function(isbreak: boolean, type?: string | null): void
{
	...
	if(!apiutil.isSafeString(_type) || apiutil.compareCaseString('text', apiutil.getSafeString(_type))){
		_is_gzip = false;
	...
	cliutil.getConsoleInput('Register URI path                     : ', true, false, function(isbreak: boolean, registerpath?: string | null): void
	{
		if(isbreak){
			process.exit(0);
		}
		const _registerpath = apiutil.getSafeString(registerpath);
