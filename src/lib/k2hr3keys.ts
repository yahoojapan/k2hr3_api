...
//
export interface K2hr3Keys {
    [key: string]: string;
}

export const getK2hr3Keys = (user?: string | null, tenant?: string | null, service?: string | null): K2hr3Keys => {
	const keywords: K2hr3Keys = {
		// common types
		'TYPE_ROLE':					'role',
	...
	const _user		= apiutil.getSafeString(user).toLowerCase();
	const _tenant	= apiutil.getSafeString(tenant).toLowerCase();
	const _service	= apiutil.getSafeString(service).toLowerCase();
