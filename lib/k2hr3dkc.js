	...
	if(input_info instanceof Object){
		if(input_info instanceof Array){
			...
			if(apiutil.isSafeString(ipaddr)){
				result.push({ip: ipaddr, hostname: null, port: portnum, cuk: cuk, extra: extra});
			}
			if(apiutil.isSafeString(hostname)){
				result.push({ip: null, hostname: hostname, port: portnum, cuk: cuk, extra: extra});
			}
		}
	}else{
		...
		result.push({ip: ipaddr, hostname: hostname, port: portnum, cuk: cuk, extra: null});
	}
	return result;
}
...
//
function rawAddHost(tenant, role, service, hostname, ip, port, cuk, extra)
{
	var	resobj = {result: true, message: null};
...
//
// return			:	always following object.
...
//
// [NOTE]
// This function is only used from rawGetResource(), and do not use this directly.
...
function rawGetRoleHostLists(dkcobj_permanent, role_key, is_expand, base_role_top)
{
	...
	subkeylist	= dkcobj_permanent.getSubkeys(name_key, true);
	if(!apiutil.isEmptyArray(subkeylist)){
		// parse hostname from 'hostname port cuk' which is subkey name.
		var	hostptn	= new RegExp('^' + keys.MATCH_ANY_HOSTNAME_PORT);							// regex = /^yrn:yahoo:(.*)::(.*):role:(.*)/hosts/name/(.*) (.*) (.*)/

		for(cnt = 0; cnt < subkeylist.length; ++cnt){
			matches	= subkeylist[cnt].match(hostptn);
			if(apiutil.isEmptyArray(matches) || matches.length < 7 || !apiutil.isSafeString(matches[4]) || !apiutil.isSafeString(matches[5])){
				continue;
			}
			//
			...
			if(rawIsPortAny(matches[5])){
				...
				// something wrong in port number, but continue without port...
				hostvalue = matches[4];															// "<hostname>"
			}

			// get "extra" data if exists
			extra	= '';
			skeyval	= dkcobj_permanent.getValue(subkeylist[cnt], null, true, null);				// get value -> yrn:yahoo:<service>::<tenant>:role:<role>{/<role>{...}}/hosts/name/"hostname port cuk"
			if(apiutil.checkSimpleJSON(skeyval)){
				skeyval = JSON.parse(skeyval);
				if(apiutil.isSafeString(skeyval.extra)){
					extra	= ' ' + apiutil.getSafeString(skeyval.extra);
				}
			}
			namevalue = matches[4] + keys.VALUE_HOST_SEP + apiutil.getSafeString(matches[5]) + keys.VALUE_HOST_SEP + apiutil.getSafeString(matches[6]) + extra;

			// add hostname to hostnames array
			apiutil.tryAddStringToArray(resultobj.normal.hostnames, hostvalue);					// for normal
			apiutil.tryAddStringToArray(resultobj.all.hostnames, namevalue);					// for all
		}
	}
	...
	subkeylist	= dkcobj_permanent.getSubkeys(ip_key, true);
	if(!apiutil.isEmptyArray(subkeylist)){
		// parse hostname from 'ip port cuk' which is subkey name.
		var	ipptn	= new RegExp('^' + keys.MATCH_ANY_IP_PORT);									// regex = /^yrn:yahoo:(.*)::(.*):role:(.*)/hosts/ip/(.*) (.*) (.*)/

		for(cnt = 0; cnt < subkeylist.length; ++cnt){
			matches	= subkeylist[cnt].match(ipptn);
			if(apiutil.isEmptyArray(matches) || matches.length < 7 || !apiutil.isSafeString(matches[4]) || !apiutil.isSafeString(matches[5])){
				continue;
			}
			//
			// matches[4] ---> ip address
			// matches[5] ---> port number
			// matches[6] ---> container unique key(now does not check this value)
			//
			if(rawIsPortAny(matches[5])){
			...
			// get "extra" data if exists
			extra	= '';
			skeyval	= dkcobj_permanent.getValue(subkeylist[cnt], null, true, null);				// get value -> yrn:yahoo:<service>::<tenant>:role:<role>{/<role>{...}}/hosts/ip/"hostname port cuk"
			if(apiutil.checkSimpleJSON(skeyval)){
				skeyval = JSON.parse(skeyval);
				if(apiutil.isSafeString(skeyval.extra)){
					extra	= ' ' + apiutil.getSafeString(skeyval.extra);
			...
			// add hostname to hostnames array
			apiutil.tryAddStringToArray(resultobj.normal.ips, hostvalue);						// for normal
			apiutil.tryAddStringToArray(resultobj.all.ips, namevalue);							// for all
		}
	}
...
function rawCreateResourceByRole(role, tenant, resourcename, type, data, resource_keys)
{
	...
	tenantname	= rolematches[2];															// not used
		...
		for(var cnt = 0; cnt < templvararr.length; ++cnt){
			...
			if(null != (matches = templvararr[cnt].match(ptnResKey)) && !apiutil.isEmptyArray(matches) && 5 <= matches.length){
				...
				if(tenant !== matches[2] || (null !== service && service !== matches[1]) || (null === service && apiutil.isSafeString(matches[1]))){
					...
					resdata[keys.VALUE_KEYS_TYPE][templvararr[cnt]] = var_host_res.normal.hostnames;
				}
				...
				if(tenant !== matches[2] || (null !== service && service !== matches[1]) || (null === service && apiutil.isSafeString(matches[1]))){
					...
					resdata[keys.VALUE_KEYS_TYPE][templvararr[cnt]] = var_host_res.normal.ips;
				}
...
//							"object"(=only object data), "keys"(=key), "aliases"(=aliases)
...
exports.addHost = function(tenant, role, hostname, ip, port, cuk, extra)
{
	// [NOTE]
...
exports.getK2hdkc = function(autorejoin, no_giveup_rejoin)
{
	...
	var	dkcobj = k2hdkc(dkcconf, dkcport, autorejoin, no_giveup_rejoin);
	if(!rawInitKeyHierarchy(dkcobj)){
...
{
	var	keys		= r3keys();
	var	dkcobj		= k2hdkc(dkcconf, dkcport, true, false);
	if(!rawInitKeyHierarchy(dkcobj)){
		dkcobj.clean();
		return null;
	}
