...
//			'host_all':		'<hostname(ip)> <port> <cuk> <extra>'			if any port, port is *
...
function rawGetRoleHostListsEx(dkcobj_permanent, keylist, is_hostname)
{
	...
	if(!apiutil.isEmptyArray(keylist)){
		...
		for(var cnt = 0; cnt < keylist.length; ++cnt){
			...
			onehost.host_all	= host + keys.VALUE_HOST_SEP + (is_any_port ? keys.VALUE_ANY_PORT : String(port)) + keys.VALUE_HOST_SEP + (null === cuk ? '' : cuk) + (null === extra ? '' : (keys.VALUE_HOST_SEP + extra));
