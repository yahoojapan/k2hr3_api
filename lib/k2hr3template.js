...
var VariableFormulaObject = function()
{
	...
	this._length= false;						// array or object length property, example '.length'
	...
	if(!this.isError() && this.isAssemble()){
		switch(type){
			...
			case	TEMPLATE_RESULT_TYPE_NUMBER:
				...
				result = (apiutil.isSafeString(this._name) || this._length || ('boolean' !== typeof this._value && null !== this._value && !isNaN(this._value)));
				break;
...
VariableFormulaObject.prototype.execute = function(varmap)
{
	...
	if(apiutil.isSafeString(this._name)){
		...
		if(undefined === varmap[this._name]){
			...
			var	length = 0;
			if(apiutil.isArray(varmap[this._name])){
				length = varmap[this._name].length;
			}else if(apiutil.isSafeEntity(varmap[this._name]) && 'object' === typeof varmap[this._name]){
				length = Object.keys(varmap[this._name]).length;
			}else{
				r3logger.dlog(this.getType() + ' class object could not make length for varmap[' + this._name + '][' + index + '] which is not array nor object, then it set 0.');
			}
	...
	if(FORMULA_SYNTAX_TYPE_IF === this.getSyntaxType() || FORMULA_SYNTAX_TYPE_ELIF === this.getSyntaxType()){
		...
		if(!this.hasLeftTemplObject() || !this.hasAuxTemplObject()){
			...
			if(apiutil.isEmptyArray(varnamearr) || 0 === varnamearr.length){
				...
				// allow array and object
				if(null === mastervar || !apiutil.isSafeEntity(mastervar) || 'object' !== typeof mastervar){
			...
			var	mastervarkeys = [];
			if(null !== mastervar){
				mastervarkeys = Object.keys(mastervar);
			}
			for(cnt = 0; null !== tmpvarname && cnt < mastervarkeys.length; ++cnt){
				// set temporary variable name and value into varmap
				var	varkey = mastervarkeys[cnt];
				if(!apiutil.isSafeEntity(mastervar[varkey]) && null !== mastervar[varkey]){
					r3logger.wlog('master variable at key/pos(' + varkey + ') is not safe entity, then skip this.');
					continue;
				}
				varmap[tmpvarname] = varkey;
...
var K2hr3TemplateEngine = (function()
{
	...
	//
	proto.load = function(templString, callback)
	{
		...
		if(!apiutil.isSafeString(temporaryString)){
			_errorstr = 'template String is empty or wrong string : ' + JSON.stringify(temporaryString);
		...
		if(TEMPL_K2HR3_TEMPLENGINE_KW !== parsers[0]){
			_errorstr = 'template String is not k2hr3 template string : ' + JSON.stringify(temporaryString);
			r3logger.elog(_errorstr);
			if(null === _callback){
				return false;
			}
		...
		if(isEmptyStatementsArray(statementsArray)){
			_errorstr = 'templateArray is empty or wrong : ' + JSON.stringify(statementsArray);
			r3logger.elog(_errorstr);
			if(null === _callback){
				return false;
			}
		...
		while(0 < statementsArray.length){
			...
			if(!resobj.result){
				_errorstr = 'failed to build statement object array : ' + resobj.message;
				r3logger.elog(_errorstr);
				if(null === _callback){
					return false;
				}
	...
	//
	proto.getVariableNames = function(callback)
	{
		var	_vararr		= [];
		var	_callback	= callback;
		var	_errorstr	= null;
		...
		if(!this.isLoaded()){
			_errorstr = 'this object does not load template string yet.';
	...
	//
	proto.execute = function(varmap, callback)
	{
